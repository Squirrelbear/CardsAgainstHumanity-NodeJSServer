...
class GameSessionManager {
    ...
    handleAction(rawData) {
        ...
        } else if(actionQuery.action === 'playCards') {
            return this.handlePlayCards(actionQuery);
        } else if(actionQuery.action === 'chooseWinner') {
            return this.handleChooseWinner(actionQuery);
        }
    }
    ...
    handleStartGame(actionQuery) {
        ...
        if(userInGame === undefined) {
            ...
            return session.getDataForPlayer(userInGame.playerID);
        }
    }

    // Plays up to 3 cards with validation of the session and user.
    handlePlayCards(actionQuery) {
        let session = this.getSessionByCode(actionQuery.sessionCode);
        if(session === undefined) {
            return {error : "Server not found. May have expired or incorrect code."};
        }
        ...
        if(userInGame === undefined) {
            ...
            let cards = [];
            if(actionQuery.playCard1) {
                cards.push(actionQuery.playCard1);
                if(actionQuery.playCard2) {
                    cards.push(actionQuery.playCard2);
                    if(actionQuery.playCard3) {
                        cards.push(actionQuery.playCard3);
                    }
                }
            }

            if(cards.length !== session.currentRound.question.numAnswers) {
                return{error : "Invalid number of cards played."};
            }
            session.playCards(userInGame.playerID, cards);
            return session.getDataForPlayer(userInGame.playerID);
        }
    }

    // Authenticates the user and then selects the winner based on a passed winnerID
    handleChooseWinner(actionQuery) {
        let session = this.getSessionByCode(actionQuery.sessionCode);
        if(session === undefined) {
            return {error : "Server not found. May have expired or incorrect code."};
        }
        ...
        if(userInGame === undefined) {
            ...
            session.selectWinner(actionQuery.winnerID);
            return session.getDataForPlayer(userInGame.playerID);
        }
    }
}
